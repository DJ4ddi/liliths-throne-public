...
//		}
	...
	public String getIcon(String context, boolean home) {
		if(isRaceConcealed()) {
			return IconCache.INSTANCE.getIcon(context, "statusEffects/raceUnknown.svg", Colour.RACE_UNKNOWN);
		} else {
			return getSubspecies().getIcon(context, home);
		}
	}
		...
		if (hasArtwork() && Main.getProperties().hasValue(PropertyValue.artwork)) {
			...
			infoScreenSB.append(
					"<div class='full-width-container' style='position:relative; float:right; width:"+percentageWidth+"%; max-width:"+width+"; object-fit:scale-down;'>"
							...
							+"</div>"
							+ "<div class='title-button' id='ARTWORK_INFO' style='background:transparent; left:auto; right:4px;'>"+IconCache.INSTANCE.getIcon("characterinfo", "UIElements/information.svg", Colour.BASE_BLACK)+"</div>"
							+ "</div>"
